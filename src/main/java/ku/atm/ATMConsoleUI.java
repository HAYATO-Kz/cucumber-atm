...
public class ATMConsoleUI {
	public void start() {
		...
		theBank.addCustomer(new Customer(1, 111, 100,100));
        theBank.addCustomer(new Customer(2, 222, 200,0));
        theBank.addCustomer(new Customer(3, 333, 300,250));
	...
	public void readCustomers(String filename, Bank bank) {
        ...
        while (in.hasNext())
		{
			...
			double currentOverdraft = in.nextDouble();
			Customer c = new Customer(number, pin, currentBalance, currentOverdraft);
			bank.addCustomer(c);
		}
		in.close();
	}
